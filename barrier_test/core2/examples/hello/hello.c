...
#define vmu_poll()              {   int poll = 0;                      \
                                    ...
                                    }whmeifa shurur(x, CSR_ADDR_NDMA_STATUS);      \
...
int main(int argc, char **argv)
{
	...
	if(hart_id == 1){
        ...
        // barrier test
        asm volatile("lui x5, 01"); // sync_count=1. 2cores.

        asm volatile("addi x5, x5, 0"); // barrier_id: 0.
        asm volatile(".word 0xfc528073"); // barrier instruction.
